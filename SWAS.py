...
class SWAS(object):
	...
	def __init__(self, Sample_Name = 'Sample', SA_Fname = None, WA_Fname = None, SA_dict = None, WA_dict = None, **kwarg):
		"""Initialize the object. This is basically a wrapper function to initialize
		...
		#This causes problems with the SWAS Sequence class
	...
	def __getitem__(self,arg):
		"""Overide the square braket operator.
	...
	def plot_data(self, qsRange=[0,np.inf],qwRange = [0, np.inf], ysShift=1, ywShift = 1, axs = None, fig = None,\
				  xUnits = 'nm', yUnits ='m', figSize=(8,8), verbose=False, **kwargs):
		"""Plot both the SAS and WAS data in the same image. Take the same
		parameters as the individual plotting functions, and pass them over.
			Args:
				qsRange (list): the q range for the small angle plot.
				 	Defaults to [0, np.inf]
				qwRange (list): the q range for the wide angle plot.
					Defaults to [0,np.inf]
				ysShift (int): the vertical shift for the small angle curve.
					Defaults to 0
				ywShift (int): the vertical shift for the wide angle curve.
					Defaults to 0
				ax (list): list with the 2 axis for plotting the SAS and WAS
					curves. The list must have 2 axes. If None create the axes.
					Defaults to None
				fig (matplotlig.figure): the figure onto which the data is
					plotted. If None create the figure and axis using the
					'figSize' keyword. Defaults to None
				xUnits (string): units for the x-axis. Defaults to 'nm'
				yUnits (string): units for the y-axis. Defaults to 'm'
				figSize (tuple): The size of the image onto which the data is
					plotted. Ignored if a figure is provided. Defaults to (8,8).
			Returns:
	...
	def get_Temp(self):
		if self.avlbCurves is 'Both':
			Temps = self.SAS._setup['Temp']
			Tempw = self.WAS._setup['Temp']
			if Temps == Tempw:
				return Temps
			else:
				logger.error('The two curves have a different temperature, \
				cannot return a unique value')
				return None
	...
	def set_Temp(self, Temp):
		if self.SAS is not None:
			self.SAS._setup['Temp'] = Temp
		if self.WAS is not None:
			self.WAS._setup['Temp'] = Temp

	def get_Dt(self):
		if self.avlbCurves is 'Both':
			Dts = self.SAS._setup['Dt']
			Dtw = self.WAS._setup['Dt']
			if Dts == Dtw:
				return Dts
			else:
				logger.error('The two curves have a different times, cannot \
				return a unique value')
				return None
		elif self.avlbCurves is 'SAS':
			return self.SAS._setup['Dt']
		elif self.avlbCurves is 'WAS':
			return 	WAS._setup['Dt']

	def set_Dt(self, Dt):
		if self.SAS is not None:
			self.SAS._setup['Dt'] = Dt
		if self.WAS is not None:
			self.WAS._setup['Dt'] = Dt
