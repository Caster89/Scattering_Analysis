...
class CentralWidget(QtWidgets.QWidget):
    ...
    will then decide how to provide the data. The plots themselves are places in
    a stacked widget, allowing to have multiple plots and switching between them
    using the dropdown menu at the bottom
    """
    ...
    def add_plot(self, plotData, plotLayout = 'individual', **kwargs):
            ...
            plotData(dict): contains all the information relative to the object
                to plot:
                    ...
                    'WAS': the list of WAS objects which needs to be plotted. (Valid
                        only if the objects is a SWAS or SWAS_Sequence object)
                    If WAS and SAS are both in the dictionary but are different,
                    then the plot will not be done and an error message will be
                    printed
        """
        ...
        if isinstance(plotData['Object'], SWAS_Sequence):
            if plotData['SAS'] or plotData['WAS']:
                #Plot both the SAS and WAS patterns are present
                if plotData['SAS'] == plotData['WAS']:
                    if len(plotData['SAS']) == 1:
                        logging.debug('plotting single SWAS')
                        ...
                        logging.debug('plotting multiple SWAS')
                ...
                #Plot only SAS data
                    ...
                    if len(plotData['SAS']) == 1:
                        logging.debug('plotting single SAS')
                        ...
                        logging.debug('plotting multiple SAS')
                ...
                #Plot only WAS data
                    ...
                    if len(plotData['WAS']) == 1:
                        logging.debug('plotting single WAS')
                ...
                logging.error('No recognizable element selected')
                return

    def add_fit(self, plotData, fitType, **kwargs ):
        '''add_fit is used to add a new plot containing the fitted data. In this case
        ...
        if isinstance(plotData['Object'], SWAS_Sequence):
            ...
            logging.info('No recognizable element selected')
            return
    ...
    def selected_plot(self):
        ...
        logging.debug('changed index to {}'.format(selIndx))
...
class SinglePlot(QtWidgets.QWidget):
    ...
    '''
    def __init__(self, parent=None, plotData = None,**kwargs):
        if kwargs.get('verbose', False):
			logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.DEBUG)
        else:
			logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.INFO)
        ...
        self.scatteringObject = plotData
        ...
        #If the widget is created with an Object than it can directly be plotted
    ...
    def remove_line(self, line):
        ...
        if isinstance(line, lines.Line2D):
            if line in self.plot_canvas.axes.lines:
                ...
                logging.error('{} is not in the axes'.format(line))
    ...
    def remove_line(self, line):
        ...
        if isinstance(line, lines.Line2D):
            if line in self.plot_canvas.axes1.lines:
                ...
                logging.error('{} was not found in either axis'.format(line))
...
class MultiplePlot(QtWidgets.QWidget):
    ...
    '''
    def __init__(self, parent=None, plotType = 'SAS', fitType = None, **kwargs):
        if kwargs.get('verbose', False):
			logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.DEBUG)
        else:
			logging.basicConfig(format='%(levelname)s:%(message)s', level=logging.INFO)
    ...
    def InitializeValues(self):
        ...
        #Defaults to SAS unless WAS is specified
        print 'Plotting Widgets is initializing values from {}'.format(self.plotData)
    ...
    def plot_data(self, **kwargs):
        ...
        if self.plotType == 'SAS':
            ...
            if isinstance(self.scatteringObject,SWAS_Sequence):
                ...
                logging.error('Cannot plot Small angles for the selected scattering data')
        if self.plotType == 'WAS':
            ...
            if isinstance(self.scatteringObject,SWAS_Sequence):
                ...
                logging('Cannot plot wide angles for the selected scattering data')
    ...
    def remove_line(self, line):
        if isinstance(line,lines.Line2D):
            ...
            logging.error('{} is not a matplotlib 2D line'.format(line))
