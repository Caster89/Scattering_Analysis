    ...
    def __init__(self, parent, scatteringObject, **kwargs):
        ...
        if isinstance(scatteringObject,SWAS):
            ...
            if scatteringObject.WAS is  None:
                self.child(_WAS).setFlags(self.child(_WAS).flags() ^ QtCore.Qt.ItemIsUserCheckable)
        #If it's a sequence then a different function is called based on whether
        #the data is sorted my angle (SAS-WAS) or by time
...
class Selectable_TreeWidget(QtWidgets.QTreeWidget):
    ...
    def fit(self, plot_widget):
        ...
        print 'The selected item is of type {}'.format(type(checkedItem['Object']))
        if isinstance(checkedItem['Object'], SmallAngleScattering):
            angles = 'SAS'
        elif isinstance(checkedItem['Object'], WideAngleScattering):
            angles = 'WAS'
        else:
            ...
            logging.debug('Selected {} angles'.format(angles))
