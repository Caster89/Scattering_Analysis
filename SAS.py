from config import _AvlbUnits, _UnitsSymbols,_UnitsConv, _AvlbSASFit, _AvlbSASFitDic, _AvlbSASFitDicInv, _lmfitModels, _lmfitModelFunctions, _lmfitDistFunctions, _lmfitModelParams,_lmfitModelsMapping
...
from Models.Utils import create_parameters
...
class SmallAngleScattering(ScatteringObject):
    ...
    def fit_data(self, fitType = "Sing_Gauss", paramSugg = None, **kwargs):
        ...
        if fitType in _lmfitModels:
            ...
            if paramSugg:
                for reqParam, paramValues in _lmfitModelParams[fitType].items():
                    logger.debug('{fitType} requires parameter {param_name}'.format(fitType=fitType, param_name = reqParam))
                    if not paramSugg.get(reqParam, None):
                        logger.debug('{param_name} not found in the provided list. Adding it...'.format(param_name=reqParam))
                        paramSugg[reqParam]= paramValues
                params = create_parameters(paramSugg)
            else:
                params = create_parameters(_lmfitModelParams[fitType])
    ...
    def lmfit_fit(self, fitType, params, qRange = np.array([0,np.inf]),
        ...
        model = getattr(Models, _lmfitModelsMapping[fitType])()
